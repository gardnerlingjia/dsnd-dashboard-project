...
python_package = project_root / "python-package"
for path in [report_dir, python_package]:
    if str(path) not in sys.path:
        sys.path.insert(0, str(path))

# Import QueryBase, Employee, Team from employee_events
from employee_events import Employee, Team, QueryBase  # type: ignore
...
def update_dropdown(r):
    ...
    print(
        f"DEBUG /update_dropdown - profile_type: {profile_type}, query_params: {dict(r.query_params)}"
    )
...
async def update_data(r):
    ...
    # Debug logging
    print(
        f"DEBUG /update_data - profile_type: {profile_type}, id: {id}, all_data: {dict(data._dict)}"
    )
